...
\begin{itemize}
    \item \textit{For KNN and SVM:} StandardScaler or MinMaxScaler applied to normalize the high-dimensional feature space
    \item \textit{For Decision Trees/Logistic Regression:} (TODO check this) Scaling may or may not be applied depending on the specific library implementation
...
Due to computational constraints, we performed strategic column removal:
    TODO confirm these match the actual pre-processing function
\begin{itemize}
    ...
    \item \textbf{High-missing columns:} Removed \texttt{2nd\_Road\_Class} (48\% missing), \texttt{Junction\_Control} (48\% missing), \texttt{Propulsion\_Code} (23\% missing)
\end{itemize}

\textbf{Missing Values:}
After removing high-missing columns, remaining NaN values were preserved rather than imputed. This approach:
\begin{itemize}
    \item Maintains data integrity by not introducing artificial patterns
    \item Is compatible with tree-based methods that handle missing values natively
    \item TODO was NaN natively compatible with KNN/SVM? If not, mention imputation strategy here
    \item Reflects genuine uncertainty in accident records
\end{itemize}
...
\textbf{Scaling:}
TODO should we have done scaling here for the numerical features? What about how scaling affects SVM?
No feature scaling was applied in the preprocessing pipeline. The tree-based classifier makes scaling unnecessary. For distance-based methods (KNN), scaling would be applied during model-specific preprocessing.

\textbf{Dataset Subsampling:}
Due to the dataset's large size (<TODO: insert total number of samples here> samples) and computational limitations, some experiments used random subsampling (e.g., 10,000 samples) with a fixed random seed for reproducibility. This trade-off prioritized experimental feasibility while maintaining representative data distribution.
...
TODO, don't have info here yet for KNN and SVM
...
\begin{itemize}
    \item No measurable performance difference with or without scaling
    \item Trees use threshold-based splits that are invariant to scale transformations
    \item This confirmed the theoretical expectation that tree methods don't require scaling
\end{itemize}

\subsubsection{Effect of Missing Value Handling}

\textbf{Voting Dataset:}
\begin{itemize}
    \item Preserving NaN values for "unknown" votes performed better than mean/mode imputation
    \item Decision Trees achieved 96.3\% accuracy with NaN preservation vs. 94.1\% with mode imputation -TODO is this true?
    \item This suggests that voting abstentions carry meaningful political information
\end{itemize}

\textbf{Road Safety Dataset:}
\begin{itemize}
    \item Tree-based methods handled missing values natively without performance degradation
    \item Attempted imputation strategies (mean for numerical, mode for categorical) did not improve results
    \item Removing rows with any missing values would have discarded $\sim$19\% of data, significantly reducing training set size
\end{itemize}

\subsubsection{Effect of Feature Engineering}

\textbf{Road Safety Time Period Feature:}
TODO any effect here? Dimensionality reduction is good for training performance anyway

\textbf{Road Safety Date Decomposition:}
\begin{itemize}
    \item TODO - should we have dropped date? Did it have an outsize effect on performance, given its larger cardinality?
\end{itemize}

\subsubsection{General Observations}

\begin{enumerate}
    \item \textbf{Preprocessing is algorithm-dependent:} What helps one classifier may be irrelevant or harmful to another
    \item \textbf{Domain knowledge matters:} Understanding data semantics (e.g., voting abstentions, time periods) guides better preprocessing decisions than generic statistical approaches
    \item \textbf{High-dimensional data requires careful scaling:} The curse of dimensionality amplifies the importance of normalization for distance-based methods
    \item \textbf{Feature engineering can outperform raw features:} Thoughtful transformations (time periods, date decomposition) improved performance while reducing dimensionality
\end{enumerate}

These findings motivated our final preprocessing choices for each dataset-classifier combination, prioritizing approaches that demonstrated empirical improvements in our validation experiments.
...
When setting the "weights" parameter, the computation time immerdiately multiplied. Normally, the calculation were done within a few seconds up to a minute. But when using other weights, we had to stop the calculation after 2 hours since id did not finish until then.
